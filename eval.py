...
def UnetPredict_old (model, raw_test, depth_test):
    ...
    width, height, channel = np.shape(op)
    fig = plt.figure()
    ax1 = fig.add_subplot(131)
    ax2 = fig.add_subplot(132)
    ax3 = fig.add_subplot(133)

    ax1.imshow(op.reshape(width, height, channel))
    ax2.imshow(depth_test[0].reshape( width, height))
    ax3.imshow(predictions[0].reshape(width, height))
    ...
    plt.show()
def UnetPredict_loaded_fld (model, raw_test, depth_test):
    predictions = model.predict(raw_test)
    ...
    # op = extraction_tools.transformation.raw_image_tansformation(raw_test[0])
    width, height,  channel = np.shape(raw_test[0])
    fig = plt.figure()
    ax1 = fig.add_subplot(131)
    ax2 = fig.add_subplot(132)
    ax3 = fig.add_subplot(133)

    ax1.imshow(raw_test[0].reshape(width, height, channel))
    ax2.imshow(depth_test[0].reshape( width, height))
    ax3.imshow(predictions[0].reshape(width, height))
    # ax1.imshow(op.reshape(480, 640, 3))
    # ax2.imshow(depth_test[0].reshape(480, 640))
